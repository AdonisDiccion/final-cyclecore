  ...
  <div className='w-full shadow-md'>
    <div className='border-b bg-white px-4 py-2 flex items-center justify-between'>
      <div className='font-bold text-lg tracking-wide'>
        <div className='flex items-center gap-1'>Sales This Day<BsFillPiggyBankFill className='text-yellow-500'/></div>
            ...
            <tbody key={orders._id}>
                ...
                <tr key={o._id} className='border-b'>
                    ...
                    <td className='flex items-center gap-3'>
                        <div>
                            <img src={`${import.meta.env.VITE_APP_REACT_APP_API}/product/photo/${o._id}`} alt="" className='w-16'/>
                        </div>
                        <div>
                            <h1 className='text-sm font-oswald'>{o.name}</h1>
                            <h1 className='text-xs font-oswald'>{orders.ordernumber}</h1>
                        </div>
                    </td>
                    <td className='p-2'>{orders.totalPrice.toLocaleString("en-PH", {style: "currency", currency: "PHP",})}</td>
                    <td className='p-2'>{orders.totalQuantity}</td>
                    <td className='p-2'>{moment(orders.createdAt).format("MMMM Do YYYY, h:mm:ss")}</td>
                </tr>
                ))}
            </tbody>
        ))}
